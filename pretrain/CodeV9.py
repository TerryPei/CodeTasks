...
def evaluate(args, our_model, dataloader, device, query='nl'):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                code_emb = our_model(input_ids=batch['code_token_ids'].to(device), \
                                    attention_mask=batch['code_mask_ids'].to(device), \
                                    position_ids=batch['code_pos_ids'].to(device))[1]

                comment_emb = our_model(input_ids=batch['comment_token_ids'].to(device), \
